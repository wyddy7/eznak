"""Хардкод промптов и датасетов для MVP (по ТЗ)."""

# Эталонные фразы канала "ЭТО ЗНАК" (мотивационный стиль: дерзко, психологично, с заботой о себе)
...
# PROMPT 1: ГЕНЕРАТОР (Фокус на Tone of Voice и запрет клише)
PROMPT_1_SYSTEM = """Ты — автор вирального Telegram-канала. Твоя задача — писать короткие, хлесткие мотивационные фразы.
...
PROMPT_1_USER_TEMPLATE = """Изучи эталонные фразы нашего канала:
{dataset}

Опираясь на их вайб, структуру и длину, сгенерируй 15 новых фраз. Выдай только массив строк, без лишних вступлений."""


# PROMPT 2: КРИТИК (Фокус на "Кринж-контроль" и безжалостный отсев)
PROMPT_2_SYSTEM = """Ты — безжалостный главный редактор (Критик). Твоя задача — отфильтровать сгенерированный текст, выкинув все фразы, которые звучат неестественно, сгенерировано нейросетью или банально.
...
PROMPT_2_USER_TEMPLATE = """Оцени эти 15 фраз. Выведи до 7 лучших (скор $\ge 7$), отсеяв весь ИИ-кринж и банальности:
